...
contract AuctionCreator {
    ...
    uint256[] public minPrices;
    uint256[] public finalPrices;
    uint256[] public qntys;
    uint256[] public endBlocks;
    ...
    function CreateAuction(
        ...
        qntys.push(qnty);
        endBlocks.push(block.number + duration);
    }
    ...
    function getOwnersData() public view returns (address[] memory) {
        return owners;
    }

    function getMinPricesData() public view returns (uint256[] memory) {
        return minPrices;
    }

    function getFinalPricesData() public view returns (uint256[] memory) {
        return finalPrices;
    }

    function getQntysData() public view returns (uint256[] memory) {
        return qntys;
    }

    function getBlocksData() public view returns (uint256[] memory) {
        return endBlocks;
    }

    // --- Duomenų nuskaitymas iš jau sukurtų aukcionų, kreipiantis į kontraktą, pagal indeks'ą masyve.

    function getParticipants(uint256 _auctionIndex)
        public
        view
        returns (address[] memory)
    {
        return Auction(address(auctions[_auctionIndex])).getParticipants();
    }
    ...
    function getBlocksLeft(uint256 _auctionIndex)
        public
        view
        returns (uint256)
    {
...
contract Auction {
    ...
    function placeBid(address _sender) public payable {
        require(_sender != auctionOwner, "Auction owner, cannot bid");
        require(block.number >= startBlock, "Auction has not started");
